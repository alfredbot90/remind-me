# remind-me installer
# Merges MCP server config into Claude Desktop without touching existing settings.
...
Write-Host " — Claude Desktop reminder installer"
Write-Host "  ──────────────────────────────────────────────"
Write-Host ""

# 1. Verify cron_mcp.ps1 is present
...
if (Test-Path $ConfigPath) {
    try {
        ...
        Write-Host "  ERROR: Could not parse existing config at:" -ForegroundColor Red
...
# 3. Inject (or overwrite) the remind-me entry
...
Write-Host "  OK  Added remind-me to Claude Desktop config" -ForegroundColor Green
Write-Host "      $ConfigPath" -ForegroundColor DarkGray
Write-Host ""

# 5. Restart Claude Desktop if it's running
